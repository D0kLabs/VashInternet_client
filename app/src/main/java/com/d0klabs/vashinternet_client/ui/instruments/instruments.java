...
public class instruments {
    ...
    public static int getPrice(int tab, int pos) {
        Price = instrumentsFragment.recyclerPriceList[tab][pos];
        return Price;
    }
    ...
    public static void updateInfoFromDB() {
        for (int u = 0; u < TableNames.length; u++) {
            ...
            try (Cursor instrumentsCount = MainActivity.dbInstrumentsHandler.getWritableDatabase().query(TableNames[u], new String[]{dbInstrumentsHandler.COL_INSTNAME}, null, null, null, null, null)) {
                ...
                instrumentsFragment.recyclerPriceList = new int [TableNames.length][count];
